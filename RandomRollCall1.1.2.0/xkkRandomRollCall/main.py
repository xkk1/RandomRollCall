...
def set_up():
    ...
    name_file_combobox.pack(expand=YES, fill=X, side=LEFT)    # 固定窗口位置
    ...
    global preview_LabelFrame
    preview_LabelFrame = LabelFrame(f1down, text='预览', padx=5, pady=5, bg = data['background'])
...
#self.Combo3.configure(state="disabled")
    ...
    encoding_combobox.pack(expand=YES, fill=X, side=LEFT)    # 固定窗口位置
...
def root():
    ...
    def add1():
        ...
        if now_name == '':
            ...
            while True:
                num += 1
                if now_name + str(num) not in names:
                    names.append(now_name + str(num))
                    shuffle(names) # 打乱姓名列表
                    # print(names)
                    break
            global remaining_names_Lable

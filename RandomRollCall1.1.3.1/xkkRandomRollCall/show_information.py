...
快速上手：
  ...
  2)进入随机点名，通过右键主窗口或按下“s”键或点击右上角的“设置”，进入设置页面。
  3)进入设置页面后先点击“读取新的姓名文件 或 重新读取姓名”按钮
  4)点击姓名文件最后的“选择文件”按钮，选择您所保存的姓名文件。
  5)点击设置窗口左下角的保存按钮，回到随机点名主窗口。
  6)现在可以使用随机点名了
      “随机点名”按钮将会随机显示一个姓名
...
功能：
（1）不重复点名
    每次随机点名时不会重复点名，并显示当前进度 剩余姓名/总姓名数
（2）记录累计抽取人数
    在程序主窗口左下角显示
    点击清零按钮可清零累计抽取人数
（3）姓名去重
...
（8）“+1”
    点击会增加该姓名被点到的概率
    设置方法：勾选设置窗口的‘增加被点到的概率“+1”功能’的复选框
（9）主窗口隐藏不必要的
    勾选后隐藏主窗口的字号“+”“-”、是否不重复点名、设置按钮、帮助按钮、累计抽取人数
    注：设置可从主窗口右键或按键盘“s”打开，帮助可在主窗口按“h”键打开。
（10）检查更新
    可检查是否有可用的更新
...
def show_add1_help():
    info = """“+1”功能
本功能开启后会在姓名后增加一个“+1”按钮
点击会增加该姓名被点到的概率
就是在已有名单中增加这个人的名字个数，比如本来只有“张三”，现在加上“张三1”，“张三2”，以此类推
点“+1”默认会显示“+1”提示，可以在设置里关闭
注意：“+1”功能在程序保存设置或重新进入时重置，即“+1”的状态不会保存。

其实这个功能来源于哔哩哔哩私信中提的建议
欢迎您对本程序提出意见或建议"""
    """：
甜瓜炖猪蹄 UID476661096
2022年3月30日 20:45
你好 ，我是一名高中物理老师，昨天下载了你制作的点名程序，今天课上已经开始用了，非常感谢你的分享
但我有个建议 ，点名提问的时候，如果学生回答错误的话，能否增加下次被提问到的概率
比如，在面板上加一个按钮，“+1”，然后执行：名单中增加这个人的名字个数，比如本来只有“张三”，现在加上“张三1”，“张三2”，以此类推
拜托拜托！
于是这个功能就诞生了"""
    show_information(information=info, title="什么是“+1”功能", icon=icon_file_path)
